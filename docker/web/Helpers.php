<?php
  ...
  function loadAndEnhanceLocations(string $filename) {
    $locations = array();
  ...
  function getCoordinates(string $name) {
  ...
  function getDistanceAndDuration(array $locations, array $destinations) {
    // construct params
    $allLocs = array_merge($locations, $destinations);
    $allCoords = array_map(function($o){return $o->getLongLat();}, $allLocs);
    $srcs = array_map(function($o, $k){return $k;}, $locations, array_keys($locations));
    $dests = array_map(function($o, $k)use($locations){return ($k+count($locations));}, $destinations, array_keys($destinations));
